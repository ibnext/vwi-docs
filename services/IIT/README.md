...
1. Het starten van de service.
   ...
   **Note:** De peildatum wordt voor het onderzoek altijd teruggezet naar de eerste dag van de maand van de peildatum zodat alleen hele maanden worden meegenomen.
   ...
   | bronbevraging_notification | `object` | Nee        | Bevat fout- of waarschuwingsmelding indien er een error is opgetreden |
   ...
   In het geval van een error in de bevraging zullen beide response variabelen niet aanwezig zijn maar bevat de response de variabele `bronbevraging_notification`.

   ### bronbevraging_notification (object)

   | Variable | Type     | Verplicht? | Toelichting                               |
   | -------- | -------- | ---------- | ----------------------------------------- |
   | message  | `string` | Ja         | Beschrijving van de fout of waarschuwing  |
   | status   | `string` | Ja         | Status van de melding: info/warning/error |

   De `bronbevraging_notification` voor bevragingen van het UWV kan twee verschillende berichten (message) bevatten:

   | Error response message                  | Toelichting                                                         |
   | --------------------------------------- | ------------------------------------------------------------------- |
   | `Bronbevraging mislukt`                 | Indien de bronbevraging mislukt door een technische reden           |
   | `Geen inkomstengegevens bekend bij UWV` | De bronbevraging voor UWV gegevens is gelukt maar zonder resultaten |
...
2. Het vervolgen van de service (stap 2).
   ...
   In het geval van een error tijdens deze stap zullen de verwachte response variabelen ontbreken. De onderstaande response variabelen zijn dan mogelijk:

   | Variable                  | Type     | Verplicht? | Toelichting                                                 |
   | ------------------------- | -------- | ---------- | ----------------------------------------------------------- |
   | netto_worker_notification | `object` | Nee        | Melding bij mislukte berekening van netto inkomen per maand |
   | inhoudingen_notification  | `object` | Nee        | Melding bij mislukte verzoek aan de Inhoudingenservice      |

   Beide notification objecten bevatten de volgende velden:
   ...
   Mogelijke error berichten

   | Error response message                       | Toelichting                                              |
   | -------------------------------------------- | -------------------------------------------------------- |
   | `Berekening netto inkomen per maand mislukt` | De berekening van het netto inkomen per maand is mislukt |
   | `Verzoek Inhoudingenservice mislukt`         | Het verzoek aan de Inhoudingenservice is mislukt         |
